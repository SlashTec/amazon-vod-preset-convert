...
if not args.interactive and (args.region is None or args.etsid is None or args.outputtype is None):
    parser.print_help()
    exit()
...
def validation_region():
    if not args.interactive:
        ...
        if region in ['us-east-1', 'us-west-1', 'us-west-2', 'eu-west-1', 'ap-southeast-1', 'ap-southeast-2', 'ap-south-1', 'ap-northeast-1']:
            return region
        else:
        ...
        while True:
            region = input("Please type in a supported ETS region: ").lower().strip()
            if region in ['us-east-1', 'us-west-1', 'us-west-2', 'eu-west-1', 'ap-southeast-1', 'ap-southeast-2', 'ap-south-1', 'ap-northeast-1']:
                return region
...
def validation_preset():
    if not args.interactive:
        try:
            ...
            etsclient = boto3.client('elastictranscoder', region_name=args.region)
        ...
        while True:
            presetid = input("Preset ID: ").lower().strip()
            try:
                etsclient = boto3.client('elastictranscoder', region_name=args.region)
...
def validate_video(ets_preset_payload, unsupported):
    if 'Video' in ets_preset_payload['Preset']:
        if json.dumps(ets_preset_payload['Preset']['Video']['Codec']) in unsupported:
            print('Unsupported Video codec found in preset, please try another preset')
            exit()
        else:
...
def validate_audio(ets_preset_payload, unsupported):
    if 'Audio' in ets_preset_payload['Preset']:
        if json.dumps(ets_preset_payload['Preset']['Audio']['Codec']) in unsupported:
            print('Unsupported Audio codec found in preset, please try another preset')
            exit()
        else:
...
def validate_output(outputtype):
    if args.interactive:
        while True:
            emf_outputgroup = input("Please type in output a output group type you want to place this ETS preset into, options are file, apple, dash, smooth: ").lower().strip()
            if emf_outputgroup in outputtype:
                return emf_outputgroup
    else:
        if args.outputtype.lower() in outputtype:
            return args.outputtype.lower()
        else:
...
def translate_audio(ets_preset_payload, s_audio):
    ...
    if ets_audio_sample == "auto":
        ...
        ets_audio_sample = int(json.dumps(ets_preset_payload['Preset']['Audio']['SampleRate']).strip('"'))
...
def translate_video(ets_preset_payload, s_video):
    ...
    ### ColorSpace Conversion Preprocessor
    ...
    if 'MaxBitrate' in videodump:
        ...
        if ets_preset_payload['Preset']['Video']['BitRate'] != 'auto':
            ...
            if emf_bitrate < 1000:
                print("WARNING: Bitrate must be greater than 1000, increasing to 1000\n")
                emf_bitrate = 1000
        else:
            emf_bitrate = 5000000
            VideoSettings[xSettings].update({'RateControlMode': emf_control_mode})
...
def translate_container(emf_AudioDescription, emf_VideoDescription, s_container, emf_outputgroup, s_video):
    if emf_VideoDescription == 'none' and s_container != '"mp4"':
        print("Audio only is supported in MP4 containers\n")
        exit()
    ...
    if emf_outputgroup == 'apple' and s_container == '"ts"':
        ...
        if emf_VideoDescription != 'none':
            OutputGroupSettings['Settings'].update(emf_VideoDescription)
        if emf_AudioDescription != 'none':
        ...
        exit()
    elif emf_outputgroup == 'apple' and s_container != '"ts"':
    ...
    if emf_outputgroup == 'dash' and s_video == '"H.264"' and s_container == '"fmp4"':
        ...
        if emf_AudioDescription != 'none':
            OutputGroupSettings['Settings'].update(emf_AudioDescription)
        return OutputGroupSettings
    elif emf_outputgroup == 'dash' and s_container != '"fmp4"':
    ...
    if emf_outputgroup == 'smooth' and s_video == '"H.264"' and s_container == '"fmp4"':
        ...
        if emf_AudioDescription != 'none':
            OutputGroupSettings['Settings'].update(emf_AudioDescription)
        return OutputGroupSettings
    elif emf_outputgroup == 'smooth' and s_container != '"fmp4"':
        print("ETS Preset must have container set to fmp4 for smooth conversion")
        exit()
...
if ets_preset_payload['Preset']['Description'] is None:
    ts = time.time()
...
if len(ets_preset_payload['Preset']['Name']) > 40:
    ...
    print("WARNING: Name is greater than 40 characters, truncating... \n")
else:
...
if args.verbose:
    ...
    if args.save:
        print('==================SAVING FILES=========================')
        with open(etsid + ".json", "w") as file:
        ...
        if s_video != 'none':
            ...
            if args.save:
                with open(etsid + "_Thumbnail.json", "w") as file:
